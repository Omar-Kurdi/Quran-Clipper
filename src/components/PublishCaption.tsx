...
export const PublishCaption: React.FC<PublishCaptionProps> = ({
  ...
  /** See `creditedTranslationNames` -- drawn on the card, not merely chosen. */
  const translationNames = useMemo(
    () => creditedTranslationNames(catalogue.options, translationIds, publish.verses, publish.wordByWord),
    [catalogue.options, translationIds, publish.verses, publish.wordByWord]
  );
